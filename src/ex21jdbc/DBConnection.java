		...
		if(con!=null) {
			...
			/*
			 3]쿼리문작성
			 	...
			 	스페이스를 삽입하는 것이 좋다는 것이다. 그렇지 않으면
			 	서로 문장이 이어지게 되어 SyntaxError가 발생된다.
			 */
			String sql = "select * from employees where "//공백
					+ " department_id=50 "
			...
			while(rs.next()) {
				...
				/*
				 ...
				 쓸 수도 있다.
				 */
				String emp_id = rs.getString(1);//employee_id컬럼(인덱스1번)
